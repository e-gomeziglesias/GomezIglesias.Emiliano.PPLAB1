...
int getString(char dato[], char msg[], char errorMsg[], int len)
{
    ...
    if (dato != NULL && len > 0)
    {
        ...
        {
            while (strlen(buffer)>len-1 || (isalpha(buffer[i])==isspace(buffer[i])))//con el -1 estoy restando el \O de la cadena
            {
                printf("%s",errorMsg);
                fflush(stdin);
                gets(buffer);
...
float getFloat(char msg[], char errorMsg[], float minRange, float maxRange)
{
    ...
    flagPoint = 0;
    returning = -1;
    if (minRange>FLT_MIN || maxRange<FLT_MAX)
    {
        printf("%s", msg);
        fflush(stdin);
        gets(buffer);
...
int getStringFull(char dato[], char msg[], char errorMsg[], int len)
{
    ...
    if (dato != NULL && len > 0)
    {
        ...
        for (i=0; i<strlen(buffer);i++)
        {
            while (strlen(buffer)>len-1 )//con el -1 estoy restando el \O de la cadena
            {
                printf("%s",errorMsg);
                fflush(stdin);
                gets(buffer);
